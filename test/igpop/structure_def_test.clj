...
(deftest fhirpath-rule-test
  (let [constraint-example {:constraint {:ele-1 {:description "All FHIR elements"}
...
(deftest valueset-test
  (matcho/match
   (sdef/valueset {:id "sample" :description "test"})
    {:binding {:valueSet "https://healthsamurai.github.io/igpop/valuesets/sample.html"
               :description "test"
               :strength "extensible"}}))

(deftest elements-to-sd-test
  (let [plain-elements-in {:CarePlan.subject {}
                           :CarePlan.text {}}]
...
(deftest generate-differential-test
  (testing "cardinality"
    (matcho/match
  ...
  (testing "extensions"
    (matcho/match
     (sdef/generate-differential :Patient "basic" (get-in project [:profiles :Patient :basic]))
     {:element [{}
                {:id "Patient.extension:race"      :path "Patient.extension"}
                {:id "Patient.extension:birthsex"  :path "Patient.extension"}]})))
...
(comment
  ...
  (println (keys project))

  (spit (io/file (str (System/getProperty "user.dir") "/show-project.json")) (generate-string project {:pretty true}))

  (def ad-project (loader/load-project (.getAbsolutePath (io/file "adverse"))))

  (:source ad-project)

  (sdef/generate-structure ad-project)

  (-> ad-project
      sdef/generate-structure
